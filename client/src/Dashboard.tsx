...
function Dashboard() {
  ...
  const [teamMembers, setTeamMembers] = useState<User[]>([]); // --- TEAM INFO STATE ---
  const [teamInfo, setTeamInfo] = useState({
    name: "Loading...",
    join_code: "...",
  }); // --- SORT STATE ---

  const [sortBy, setSortBy] = useState("newest"); // --- DARK MODE STATE ---
  ...
  const currentUser = storedUser
    ...
    : { id: 1, team_id: 1, username: "DevUser", email: "dev@example.com" }; // --- STATS STATE ---
  ...
  const [selectedBug, setSelectedBug] = useState<Bug | null>(null); // --- FETCH DATA --- // 1. Fetch Team Members

  useEffect(() => {
    // FIX: Changed from http://localhost:3001/... to relative path
    axios
      .get(`/bugs/team-members?team_id=${currentUser.team_id}`)
      .then((res) => setTeamMembers(res.data))
      .catch((err) => console.error(err));
  }, [currentUser.team_id]); // 2. Fetch Stats

  useEffect(() => {
    if (activeTab === "settings") return; // FIX: Changed from http://localhost:3001/... to relative path

    let endpoint = `/bugs/stats?team_id=${currentUser.team_id}`;
    if (activeTab === "my_issues")
      endpoint = `/bugs/stats?assignee_id=${currentUser.id}`;
    axios
      .get(endpoint)
      .then((res) => setStats(res.data))
      .catch(console.error);
  }, [activeTab, currentUser.id, currentUser.team_id]); // 3. Fetch Team Name & Join Code

  useEffect(() => {
    if (currentUser.team_id) {
      // FIX: Changed from http://localhost:3001/... to relative path
      axios
        .get(`/bugs/teams/${currentUser.team_id}`)
        .then((res) => setTeamInfo(res.data))
        .catch((err) => console.error("Error fetching team info:", err));
    }
  }, [currentUser.team_id]); // --- HELPERS ---
  ...
  const getPageSubtitle = () => {
    if (activeTab === "dashboard")
      return "Welcome back! Here's what's happening with your bugs today.";
    if (activeTab === "my_issues")
      return "Here are the metrics for bugs assigned specifically to you.";
    if (activeTab === "settings")
      return "Manage your profile and application preferences.";
  ...
  const handleShow = () => setShowModal(true); // --- ACTIONS ---

  const handleSaveBug = async () => {
    try {
      // FIX: Changed from http://localhost:3001/... to relative path
      await axios.post("/bugs", newBug);
      setShowModal(false);
  ...
  const handleUpdateBug = async () => {
    ...
    try {
      // FIX: Changed from http://localhost:3001/... to relative path
      await axios.put(`/bugs/${selectedBug.id}`, selectedBug);
  ...
  const handleDeleteBug = async () => {
    ...
    try {
      // FIX: Changed from http://localhost:3001/... to relative path
      await axios.delete(`/bugs/${selectedBug.id}`);
      setShowSidebar(false);
      window.location.reload();
    } catch (error) {
  ...
  const handleDeleteAccount = async () => {
    if (
      ...
      try {
        // FIX: Changed from http://localhost:3001/... to relative path
        await axios.delete(`/bugs/users/${currentUser.id}`);
    ...
    <div className="container-fluid bg-body-tertiary min-vh-100">
      ...
      <div className="row">
        ...
        <nav className="col-md-2 d-none d-md-block bg-body sidebar min-vh-100 border-end">
          ...
          <div className="p-4">
                        
            <h4 className="fw-bold text-primary mb-5">
              <i className="bi bi-bug-fill me-2"></i>BugTracker
            </h4>
                        
            ...
            <ul className="nav flex-column gap-2">
              ...
              <li className="nav-item">
                                
                <button
                  className={`btn w-100 text-start border-0 fw-medium py-2 ${
                    activeTab === "dashboard"
              ...
              <li className="nav-item">
                ...
                >
                                    <i className="bi bi-list-task me-3"></i> All
                  Issues                 
                </button>
                              
              </li>
              ...
              <li className="nav-item">
                ...
                >
                                    <i className="bi bi-person me-3"></i> My
                  Issues                 
                </button>
                              
              </li>
        ...
        <main className="col-md-10 ms-sm-auto p-4 md-p-5">
          ...
          <div className="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
              ...
              <p className="text-secondary">{getPageSubtitle()}</p>
            </div>
                                     
            {activeTab !== "settings" && (
                  ...
                  <Form>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <small className="text-muted">
                        Share this code to invite others to your team.
                      </small>
                                                    
                    </Form.Group>
                                              
              ...
              <div className="col-md-6">
                  ...
                  <div className="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div>
                      ...
                      <small className="text-secondary">
                        Receive emails when bugs are assigned to you.
                      </small>
                    </div>
                                                  
                ...
                <div className="card border-0 shadow-sm rounded-4 p-4 border-danger-subtle bg-body">
                  ...
                  <p className="text-secondary small">
                    ...
                    bugs assigned to you will be unassigned.
                  </p>
                                            
                  <Button
              ...
              {activeTab === "dashboard" && (
                <div className="row g-4 mb-5">
                  ...
                  <div className="col-md-3">
                    <div className="card border-0 shadow-sm p-4 rounded-4 h-100 bg-body">
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                    ...
                    <div className="card border-0 shadow-sm p-4 rounded-4 h-100 bg-body">
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                          ...
                          <h1 className="fw-bold mb-0">{stats.critical}</h1>
                        </div>
                        <div
                    ...
                    <div className="card border-0 shadow-sm p-4 rounded-4 h-100 bg-body">
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                          ...
                          <h1 className="fw-bold mb-0">{stats.resolved}</h1>
                        </div>
                        <div
                    ...
                    <div className="card border-0 shadow-sm p-4 rounded-4 h-100 bg-body">
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                          ...
                          <h1 className="fw-bold mb-0">{stats.in_progress}</h1>
                        </div>
                        <div
          ...
          <Modal show={showModal} onHide={handleClose} centered>
              ...
              <Form>
                ...
                <div className="row mt-3">
                  ...
                  <div className="col">
                    ...
                    <Form.Select
                      onChange={(e) =>
                        setNewBug({ ...newBug, assignee_id: e.target.value })
                      }
                    >
            ...
            <Modal.Footer>
              ...
              <Button variant="primary" onClick={handleSaveBug}>
                Save Bug
              </Button>
            </Modal.Footer>
                      
          ...
          >
            ...
            <Offcanvas.Header closeButton className="border-bottom">
              <Offcanvas.Title className="fw-bold text-secondary">
                {selectedBug ? `BUG-${1000 + selectedBug.id}` : "Details"}
              </Offcanvas.Title>
            </Offcanvas.Header>
                        
              ...
              {selectedBug && (
                <Form>
                  ...
                  <Form.Group className="mb-4">
                    <Form.Control
                      type="text"
                      className="fs-4 fw-bold border-0 px-0 shadow-none bg-transparent"
                  ...
                  <div className="d-flex gap-3 mb-4">
                    ...
                    <div className="flex-grow-1">
                      <label className="small text-secondary fw-bold mb-1">
                        STATUS
                      </label>
                      <Form.Select
                        className="fw-medium bg-body border-0"
                  ...
                  <Form.Group className="mb-4">
                    ...
                    <Form.Control
                      as="textarea"
                      rows={6}
                      className="bg-body border-0 p-3"
                  ...
                  <div className="d-flex justify-content-between pt-4 border-top">
                                                
                    <Button variant="outline-danger" onClick={handleDeleteBug}>
                      <i className="bi bi-trash me-2"></i> Delete
                    </Button>
                                                
                    <Button variant="primary px-4" onClick={handleUpdateBug}>
                      Save Changes
                    </Button>
                                            
                  </div>
